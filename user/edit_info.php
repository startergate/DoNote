...
<?php
  ...
  switch ($_SESSION['confirm']) {
    ...
    case 'confirm2':
      ...
      break;
    default:
      echo "<script>window.alert('인증이 필요합니다.NEW');</script>";
  ...
  <body>
    <div class="container-fluid" id='padding-erase'>
      <div class="fixed layer1" id="bgi" style="z-index: 2">
        <div class="col-md-3">
          <a href="../note.php" class='middle'><img src="/static/img/common/donotevec.png" alt="DoNote" class="img-rounded" id=logo alt='메인으로 가기' \></a><a href="../function/edit_info_direct.php?target=../user/edit_info.php" id="white" class="btn btn-link">유저 정보 수정</a>
        </div>
      ...
      <div class="col-md-10">
        <form action="../function/edit_user_info.php" method="post">
          <div class="form-group">
            <textarea type='text' class='form-control' name='nickname' placeholder='닉네임(비워두시면 닉네임이 변경되지 않습니다.)'></textarea>
          </div>
          <div class="form-group">
            <input type='password' class='form-control' name='pw' placeholder='비밀번호(비워두시면 비밀번호가 변경되지 않습니다.)'>
        </div>
        <div class="form-group">
            <input type='password' class='form-control' name='confirm' placeholder='비밀번호 확인(비밀번호 입력 란이 비어있다면, 입력하지 마세요.)'>
        </div>
